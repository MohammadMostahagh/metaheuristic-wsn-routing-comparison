...
from sa_engine_T3 import run_sa

N_RUNS = 30
SEED_BASE = 42

def run_multi_sa():
    print("Starting scientific multi-run experiment (Simulated Annealing)...")
    ...
    for i in range(N_RUNS):
        ...
        best_fit, history = run_sa(seed=seed)
    ...
    TEMP_STEPS = len(avg_history["max_fitness"])

    with open("sa_averaged_convergence_history.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Iteration", "MaxFitness", "AvgFitness", "TotalEnergy_J", "AvgDelay_Hops", "MinResidual_J"])
        for step in range(TEMP_STEPS):
            writer.writerow([
                step,
                avg_history["max_fitness"][step],
                avg_history["avg_fitness"][step],
                avg_history["best_energy"][step],
                avg_history["best_delay"][step],
                avg_history["best_min_res"][step]
            ])

    with open("sa_multi_run_summary.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Run", "Seed", "Best_Fitness", "Best_Energy_J", "Best_Delay_Hops", "Min_Residual_J"])
        for summary in run_summaries:
            writer.writerow(summary)

    print("\nMulti-run SA experiment finished successfully!")
    print("Important Files generated for Plotting:")
    print(" 1. sa_averaged_convergence_history.csv")
    print(" 2. sa_multi_run_summary.csv")
